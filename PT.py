...
class ligand:
    """
        ...
        This property returns the center of mass of the residue, which is generated from all atom components. 
        """
    def __init__(self, ID = None, file_path = None, chain = None, ResNum = None):
        self.sites = []
        self.atoms = []
    ...
    def download(self, directory = os.getcwd(), convert_pdb = False, file_path = None):
        ...
        if file_path != None:
            ...
            try:
                url = 'https://files.rcsb.org/ligands/' + self.ID + '_ideal.sdf'
                print(f'1: {url}')
                ...
                try:
                    print('2')
                    pcp.download('SDF', directory + '/' + self.ID + '.sdf' , self.ID, 'smiles', overwrite = True) 
                    
                except:
                    try:
                        print('3')
                        pcp.download('SDF', directory + '/' + self.ID + '.sdf' , self.ID, 'inchikey', overwrite = True)  
                    except:
                        print('Ligand identifier not found! File downloaded incorrectly')
        
            if convert_pdb == True and len(self.atoms) == 0:
                try:
                    print('obabel -isdf "' + directory + '/' + self.ID + '.sdf" -O "' + directory + '/' + self.ID + '.pdb" --gen3d')
                    os.system('obabel -isdf "' + directory + '/' + self.ID + '.sdf" -O "' + directory + '/' + self.ID + '.pdb" --gen3d')
                    with open(directory + '/' + self.ID + '.pdb', 'r') as f:
                        atoms = f.readlines()
                    os.remove(directory + '/' + self.ID + '.pdb')
                    for line in atoms:
                        if line[0:4] == 'ATOM' or line[0:6] == 'HETATM':
                            element = line[76:80].strip()
                            if element not in atom_keys:
...
class Protein:
                ...
                for count, line in enumerate(data):
                    if line[0:6] == 'HETATM':
                        ...
                        if lignum != currnum or abs(atom_number - int(line[6:12])) > 100:
                            currnum = lignum
                            lig = ligand(ligands, chain=chain, ResNum=ResNum)
    ...
    def ligands(self):
        ...
        ligcache, supp = [], []
        for ligand in self.ligand_list:
            print(f'BOH.ID: {ligand.ID}')
            if ligand.ID not in ligcache:
                print(f'BOH: {ligcache}')
                # if len(ligand.ID) < 3 or len(ligand.atoms) < 2:
                if len(ligand.atoms) < 2:
                    ions.append(ligand.ID)
                elif len(ligand.ID) >= 3 and ligand.ID[2] == '4' and len(ligand.atoms) < 7:
                    ions.append(ligand.ID)
                elif ligand.ID != 'HEM' and len(ligand.atoms) > 4:
                    ### TODO: here
                    true_ligand = False
                    try:
                        print(f'http://ligand-expo.rcsb.org/files/{ligand.ID[0]}/{ligand.ID}/ccif')
                        urllib.request.urlretrieve(f'http://ligand-expo.rcsb.org/files/{ligand.ID[0]}/{ligand.ID}/ccif', f'{ligand.ID}.txt')
                        with open(f'{ligand.ID}.txt', 'r') as f:
                            lines = f.readlines()
                        os.remove(f'{ligand.ID}.txt')

                        for line in lines:
                            line = line.strip()
                            if line.startswith('<li>'):
                                filename = line.strip().split()[-1][:-9]
                                target = filename.split('_')
                                if target[0] == self.protein.lower() and target[1] == ligand.ID and target[3] == ligand.chain and target[4] == ligand.ResNum:
                                    break
                                    
                        print(f'http://ligand-expo.rcsb.org/files/{ligand.ID[0]}/{ligand.ID}/ccif/{filename}')
                        urllib.request.urlretrieve(f'http://ligand-expo.rcsb.org/files/{ligand.ID[0]}/{ligand.ID}/ccif/{filename}', f'{ligand.ID}.icif')
                        with open(f'{ligand.ID}.icif', 'r') as f:
                            lines = f.readlines()
                        os.remove(f'{ligand.ID}.icif')

                        for line in lines:
                            line = line.strip()
                            if line.startswith('_chem_comp.pdbx_type') and line[-6:] == 'HETAIN':
                                true_ligand = True
                                break

                    except:
                        print(traceback.format_exc())
                        print(f'Unable to collect informations about {ligand.ID}, (MAYBE NOT LIGAND)!')
...
if __name__ == '__main__':
    ...
    # urllib.request.urlretrieve(f'http://ligand-expo.rcsb.org/files/H/HED/ccif/', f'tmp.icif')
    # with open(f'{os.getcwd()}/tmp.icif', 'r') as f:
    #     lines = f.readlines()
    ...
    #         filename = line.strip().split()[-1][:-9]
    #         target = filename.split('_')
    #         if target[0] == '1d3n' and target[1] == 'HED' and target[3] == 'A' and target[4] == '170':
    #             print(filename)
    #             break

    
    # urllib.request.urlretrieve(f'http://ligand-expo.rcsb.org/files/H/HED/ccif/{filename}', f'tmp.icif')
    # with open(f'{os.getcwd()}/tmp.icif', 'r') as f:
    #     lines = f.readlines()
